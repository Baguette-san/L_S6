...
let minimax e profondeur etatFils eval = let listeFils = etatFils e in 
    match listeFils with
        [] -> eval e
        | _ -> let l = (List.map (fun x -> minimax (snd x) (profondeur+1) etatFils eval) listeFils) in 
                if (profondeur mod 2) = 0 then maxip l
                else minip l
;;
 
let recherche pmax e profondeur etatFils eval = let listeFils = (if pmax > profondeur then etatFils e else []) in 
    match listeFils with
        [] -> eval e
        | _ -> let l = (List.map (fun x -> recherche pmax (snd x) (profondeur+1) etatFils eval) listeFils) in 
                if (profondeur mod 2) = 0 then maxip l
                else minip l
;;

let meilleur l = 
    let rec traitement n = function
        [] -> n
        |x::s -> if (fst x > fst n) then traitement x s
                else traitement n s
    in match l with
        [] -> failwith "liste vide"
        | x::s -> traitement x s
...
let meilleurCoup pmax e profondeur etatFils eval = let listeFils = (if pmax > profondeur then etatFils e else []) in 
    let rec aux pmax profondeur etatFils eval = function
        [] -> []
        | (c,x)::s -> (recherche pmax x profondeur etatFils eval, c) :: aux pmax profondeur etatFils eval s
    in meilleur (aux pmax (profondeur+1) etatFils eval listeFils)
;;
(* ==================================================================================== *)
...
print_newline (); print_string ("-----Minimax-----"); print_newline ();
minimax etatInitialA1 0  etatsFilsA1 evaluationA1;;
(* - : int = -1                                                                         *)
let resMinA1= -1;;
print_newline (); print_string ("-----Recherche-----"); print_newline ();
recherche 3 etatInitialA1 0  etatsFilsA1 evaluationA1;;
(* - : int = -1                                                                         *)
let resRechA1= ("p=3",-1);;
print_newline (); print_string ("-----Meilleur coup-----"); print_newline ();
