(* Hermès Desgrez Dautet TPA11 21710353*)
...
(* ajout TP *)
(*largeur*)
let rec largeur attente estBut etatsSuivants = 
match attente with
[]-> failwith "pas de but trouve"
|x::reste -> if (estBut x) 
                then( print_string "file d'attente:" ; print_list_string attente; print_newline () ; x)
                else( print_string "file d'attente:" ; print_list_string attente; print_newline () ; 
                                largeur( reste@(etatsSuivants x)) estBut etatsSuivants) 
;;

(*longueur*)
let rec longueur attente estBut etatsSuivants = 
match attente with
[]-> failwith "pas de but trouve"
|x::reste -> if (estBut x) 
                then( print_string "file d'attente:" ; print_list_string attente; print_newline () ; x)
                else( print_string "file d'attente:" ; print_list_string attente; print_newline () ; 
                                longueur( reste@(etatsSuivants x)) estBut etatsSuivants) 
;;
...
let creerLesFils etat cout opPoss hEtat =
    let listeDesFils = (opPoss etat) in
    let rec ajout cout l  hEtat = match l with
        [] -> []
        | e::s ->   begin
                    match e with cmd,coutE,nom -> 
            (nom,coutE+cout,hEtat nom) :: ajout cout s hEtat 
                    end in
    ajout cout listeDesFils hEtat
;;
        

(*insererLesFils*)
let rec insererLesFils listeDesFils listeAttente = 
    let rec tri node liste = match liste with
        [] -> [node]
        | e::s -> 
            begin 
            match node,e with 
                (nomN,coutN,heurN),(nomE,coutE,heurE) -> 
                                        if (coutN+heurN)<(coutE+heurE) 
                                        then node::liste
                                        else e::(tri node s)
            end
    in match listeDesFils with 
        [] -> listeAttente
        | e::s -> insererLesFils s ( tri e listeAttente)
;;

(* A* *)
let rec printNomCoutHeur l = match l with
    [] -> print_string ". "
    | e::s -> let nom,cout,heur = e in print_string (" ( "^nom^" "^string_of_int cout^") "); printNomCoutHeur s
;;

let rec aEtoile listeAttente estBut opPoss hEtat =
match listeAttente with
[]-> failwith "pas de but trouve"
| e::s -> let (nom,cout,heur) = e in if (estBut nom) 
                then (print_string "file d'attente:";printNomCoutHeur listeAttente, print_newline (); 
                print_string ("but trouvé: "^nom^" avec un cout de "^ string_of_int cout); print_newline ();nom,cout)
                else (print_string "file d'attente;" ; printNomCoutHeur listeAttente; print_newline();
                aEtoile (insererLesFils (creerLesFils nom cout opPoss hEtat) s ) estBut opPoss hEtat)
;;
    
 aEtoile (initG1C,0,hEtatG1C initG1C) estButG1C opPossG1C hEtatG1C;;
